...
float expressao ( float X ){
	...
	resul = (1250/X)*(1-pow((1+X), -10)) - 10000;
...
//	cin >> Xval;
	...
	while( raiz > erro || raiz < 0){
		...
		if( intervalos[0] == intervalos[1] )
			break;
		if ( i == inter ){
			cout << "Numero maximo de interacoes alcancado!\n" << endl;
			break;
		}		
		...
		//print
		cout << i+1 << "\t  " << intervalos[0] << "  \t" << intervalos[1] << "    " <<
...
//		if( med1 < erro ) //a questao nao pede
//			break;
	
		i++;
	}
	
	cout << intervalos[1] << " - " << intervalos[0] << endl << "-------------------  = " <<
	((intervalos[0] - intervalos[1])/intervalos[0])*(-1) << endl << "      " << intervalos[0];
	cout << "\n\nTaxa de erro = " << ((intervalos[0] - intervalos[1])/intervalos[0])*(-1) << endl;
